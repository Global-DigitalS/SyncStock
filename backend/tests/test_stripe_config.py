...
class TestStripeConfigEndpoints:
    ...
    def test_get_stripe_config_success(self):
        ...
        print(f"✓ GET /api/admin/stripe/config returned config structure correctly")
    ...
    def test_put_stripe_config_success(self):
        ...
        print(f"✓ PUT /api/admin/stripe/config updated config successfully")
    ...
    def test_put_stripe_config_partial_update(self):
        ...
        assert response.status_code == 200, f"Expected 200, got {response.status_code}: {response.text}"
        print(f"✓ PUT /api/admin/stripe/config accepts partial updates")
    ...
    def test_post_test_connection_without_secret_key(self):
        ...
        if response.status_code == 400:
            print(f"✓ POST /api/admin/stripe/test-connection returns 400 without secret key")
        else:
            data = response.json()
            assert data.get("success") == False, "Should indicate failure without secret key"
            print(f"✓ POST /api/admin/stripe/test-connection returns failure status without secret key")
    ...
    def test_config_persistence(self):
        ...
        print(f"✓ Config persistence verified - all values saved and retrieved correctly")
