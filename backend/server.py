...
@api_router.get("/products-unified", response_model=List[UnifiedProductResponse])
async def get_unified_products(
    ...
    """Get products grouped by EAN, showing best price from all suppliers"""
    ...
    for item in results:
        ...
        # Find best offer: lowest price with stock > 0
        offers_with_stock = [p for p in products if p.get("stock", 0) > 0]
        if offers_with_stock:
            best = min(offers_with_stock, key=lambda x: x.get("price", float('inf')))
        else:
            # If no stock anywhere, show lowest price anyway
            best = min(products, key=lambda x: x.get("price", float('inf')))
        
        # Build supplier offers list
        suppliers = []
        ...
        unified_products.append(UnifiedProductResponse(
            ...
            best_supplier_id=best.get("supplier_id", ""),
...
@api_router.get("/products-unified/{ean}", response_model=UnifiedProductResponse)
async def get_unified_product(ean: str, user: dict = Depends(get_current_user)):
    """Get a single unified product by EAN with all supplier offers"""
    
    products = await db.products.find(
        {"user_id": user["id"], "ean": ean},
        {"_id": 0}
    ).to_list(100)
